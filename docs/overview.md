...
| Niri shell | DankMaterialShell, started by systemd under Niri only; the image refreshes Quickshell's Qt runtime families and runs `qs --version` during the build so a private-ABI mismatch fails before publication (DD-070) | `files/system/usr/lib/systemd/user/niri.service.d/`, `recipe.yml` (`containerfile`) |
