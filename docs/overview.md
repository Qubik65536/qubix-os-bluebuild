...
| Packages added | `micro` (editor), `starship` (shell prompt, from COPR), `wezterm` (terminal, from COPR), `niri` (second session), `dms` + fonts + `cliphist` (Niri's shell, from COPR), Fcitx 5 + Pinyin + toolkit/KDE integration (Simplified Chinese input), Plasma Discover + Flatpak backend, `grub2-tools-extra` (PF2 font generation) | `recipe.yml` (`dnf`) |
...
| Plasma software centre | Discover with its Flatpak backend, matching Plasma's pinned stock favorite and the configured Flathub remotes | `recipe.yml` (`dnf`) |
