...
| Simplified Chinese input | Fcitx 5 with English (US) and Pinyin defaults; `Super+Space` in Plasma, `Ctrl+Space` in Niri; native startup integration and compositor-specific toolkit environments | `recipe.yml` (`dnf`, Plasma session patch), `files/system/etc/{xdg/fcitx5,profile.d}/`, `files/system/etc/xdg/kwinrc`, `files/system/etc/niri/config.kdl` |
| Plasma launcher defaults | Retains Aurora's package-free taskbar and application-launcher defaults; Discover is not installed or pinned, while personal panel configuration remains higher priority | inherited Aurora KDE profile; `recipe.yml` assertion (DD-069) |
...
| Niri shell | DankMaterialShell, started by systemd under Niri only; the image synchronizes the Qt runtime with KWin and Plasma Workspace, then smoke-tests Quickshell and Plasma's battery QML plugin so a private-ABI mismatch fails before publication (DD-070, DD-071) | `files/system/usr/lib/systemd/user/niri.service.d/`, `recipe.yml` (`containerfile`) |
