...
followed by BlueBuild's `initramfs` module.
...
- Any prebuilt out-of-tree module (`kmod-*`) inherited from Aurora is removed with the
  stock kernel: those RPMs depend on the exact `kernel-uname-r` that is going away. The
  build log lists what was present before the swap, so a regression is traceable.
