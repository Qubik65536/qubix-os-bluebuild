...
| File | Read by | Holds |
|---|---|---|
| `/etc/profile.d/qubix-shell-env.sh` | sh, bash and zsh | KDE's Qt platform theme when unset, `XDG_CONFIG_DIRS`, Homebrew's `XDG_DATA_DIRS` entry, `EDITOR`, `VISUAL`, `STARSHIP_CONFIG`, the `ATUIN_*` settings, `LG_CONFIG_FILE` — and, at the end, bash's interactive setup |
...
| Path | Purpose |
|---|---|
| `/etc/profile.d/qubix-shell-env.sh` | KDE's Qt platform theme when unset, `XDG_CONFIG_DIRS`, Homebrew's `XDG_DATA_DIRS` entry, `EDITOR`, `VISUAL`, `STARSHIP_CONFIG`, the `ATUIN_*` settings, `LG_CONFIG_FILE`, and bash's setup |
