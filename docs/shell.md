...
Distrobox guests created after this image was last rebased automatically receive the
Qubix zsh experience — Starship, Atuin, zsh-autosuggestions, zsh-syntax-highlighting,
the completions, and the shared helpers (`cat`, `y`, `lg`) — with no manual setup.

### How it works

The image ships `/etc/distrobox/distrobox.conf`, which Distrobox reads as its
system-wide default configuration. It sets `container_init_hook` to a script that
runs inside every new guest at the end of container initialisation:

1. **`qubix-distrobox-init.sh`** is the hook. It runs inside the guest as the container
   user, with `sudo` available (Distrobox configures this). It installs `zsh` via the
   guest's native package manager (`dnf`, `apt-get`, `pacman`, `zypper`, or `apk`).
   It then installs two files into the guest's `/etc/`:
   - `/etc/profile.d/qubix-distrobox-env.sh` — exports `STARSHIP_CONFIG`, the `ATUIN_*`
     variables, `LG_CONFIG_FILE`, and `EDITOR`/`VISUAL`.
   - `/etc/zshrc.d/90-qubix.zsh` (or an appended line in `/etc/zshrc` on guests that
     lack that directory) — the zsh interactive setup.

2. **At runtime**, the two files installed in the guest source their content from the
   HOST via `/run/host/usr/share/qubix-os/…`. Distrobox mounts the host root at
   `/run/host` inside every standard container. Plugins and binaries (`starship`,
...
| Guest package manager | `zsh` installed automatically |
|---|---|
...
| `zypper` (openSUSE …) | ✓ |
| `apk` (Alpine) | ✓ |
| anything else | ✗ — install `zsh` manually, then re-run the hook |

`--unshare-all` containers do not receive the `/run/host` bind mount by default. The
drop-in's guard silently skips the entire Qubix setup in that case, leaving the guest's
native shell untouched. This is intentional: an `--unshare-all` container is explicitly
isolated.

### User overrides in a guest

Everything is a default. The same override table above applies inside the guest:
`~/.config/starship.toml`, `~/.config/atuin/config.toml`, and `~/.zshrc` all work
exactly as they do on the host — and since `$HOME` is shared, any file you create on
the host is automatically visible in the guest.

To opt out of the system-wide hook entirely, add this line to `~/.distroboxrc`:

```sh
container_init_hook=""
```

Distrobox sources `~/.distroboxrc` last, so a user's value wins over
`/etc/distrobox/distrobox.conf`.

### Existing containers

Containers created **before** this image was rebased do not pick up the init hook —
Distrobox runs it once at creation time only. To bring an existing container up to date:

```sh
distrobox rm <container-name>
distrobox create --name <container-name> --image <same-image>
```

Your home directory is shared and is not deleted. Any container-local packages you
installed in `/usr` (inside the container) will be gone; recreating the container from
the same image is the clean restart.

Alternatively, to apply the setup manually inside an existing container without
recreating it:

```sh
distrobox enter <container-name>
sudo /run/host/usr/share/qubix-os/distrobox/qubix-distrobox-init.sh
```
...
| `/etc/distrobox/distrobox.conf` | System-wide Distrobox defaults; sets `container_init_hook` (DD-043) |
| `/usr/share/qubix-os/distrobox/qubix-distrobox-init.sh` | Init hook script run inside each new guest at creation time |
| `/usr/share/qubix-os/distrobox/qubix-distrobox.zsh` | Guest zsh drop-in; sourced from the host via `/run/host` at runtime |
| `/usr/share/qubix-os/distrobox/qubix-distrobox-env.sh` | Guest env file; sources Starship/Atuin config from host paths |
