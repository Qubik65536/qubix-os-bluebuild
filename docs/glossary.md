...
| **BlueBuild** | The build system used here. Transpiles a recipe in `recipes/` into a `Containerfile` and builds it. Kept in diagnostic and deployment identity, but omitted from prominent visual product names (DD-065). <https://blue-build.org> |
...
| **Flatpak seeding** | The rebase fallback: Flatpaks are not baked into the OCI image, so a systemd timer installs Qubix's browser and image viewer on first boot. Installer ISOs instead carry an offline Flatpak repository and Anaconda installs the declared desktop set (DD-061). |
...
| **Lorax build stamp** | `/.buildstamp` inside installer media. It retains the technical image product; Anaconda overlays only its UI-facing `Product` through Qubix's partial `PRODBUILDPATH` fragment (DD-065). |
