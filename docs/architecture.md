...
| 4 | `containerfile` | `common-base.yml` | Installs `zsh-completions` from a pinned upstream tag into `/usr/share/zsh/site-functions`; asserts `/usr/bin/zsh` reached `/etc/shells`; installs `zellij` from upstream's pinned `no-web` release with its completions, and asserts `/etc/zellij/config.kdl` resolves and parses | Needs `zsh` and `git` from the `dnf` module, and `/etc/zellij` from the `files` module. Neither zsh-completions nor zellij exists as a package to install — Fedora has neither, the zsh-completions COPR has no chroots, and no zellij COPR is endorsed upstream (DD-026, DD-028, DD-033). |
