...
| 4 | `containerfile` | `common-base.yml` | Installs and validates the terminal environment, builds/asserts the bounded-range GRUB PF2 payload, validates Homebrew and KDE/DMS environment wiring, asserts Aurora's package-free Plasma launcher defaults, replaces Plasma's Breeze launcher aliases with Qubix distributor artwork, and synchronizes Quickshell's Qt runtime before running its loader smoke test | Needs packages from `dnf` and overlaid configuration/scripts from `files`; late assertions turn missing assets, renamed units, lost executable bits, or a Qt private-ABI mismatch into build failures (DD-057, DD-062, DD-063, DD-066…DD-070). |
