...
| 4 | `containerfile` | `common-base.yml` | Installs and validates the terminal environment as before, then converts the installed Monaspace Krypton NF Regular/Bold faces into bounded-range GRUB PF2 fonts and generates/asserts the Qubix Boot Console manifest | Needs `grub2-mkfont` from `dnf`, Krypton NF from the earlier font snippet, plus configuration, artwork, and scripts from `files`. The GRUB payload must be complete before its boot-time installer can copy it (DD-057). |
| 5 | `systemd` | `common-base.yml` | Enables `qubix-default-shell.service` and `qubix-grub-theme.service` | Both units come from `files`: one changes existing accounts once (DD-035); the other synchronises image-owned GRUB assets into machine-local `/boot` without regenerating menu entries (DD-057). |
