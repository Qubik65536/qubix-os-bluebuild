...
| 4 | `containerfile` | `common-base.yml` | Installs and validates the terminal environment as before, then converts IBM Plex Mono into GRUB PF2 fonts and generates/asserts the Qubix Boot Console manifest | Needs tools and fonts from `dnf`, plus configuration, artwork, and scripts from `files`. The GRUB payload must be complete before its boot-time installer can copy it (DD-057). |
