...
| 4 | `containerfile` | `common-base.yml` | Installs and validates the terminal environment, builds/asserts the bounded-range GRUB PF2 payload, validates Homebrew and KDE/DMS environment wiring, then validates Discover and replaces Plasma's Breeze launcher aliases with Qubix distributor artwork | Needs packages from `dnf` and overlaid configuration/scripts from `files`; late assertions turn missing assets, renamed units, or lost executable bits into build failures (DD-057, DD-062, DD-063, DD-066…DD-068). |
