...
- **Composition:** `from-file: common-base.yml` → `from-file: common-identity.yml` →
  ...
  | 4 | `containerfile` | `common-base.yml` | Thirteen snippets: nine terminal/setup assertions, bounded-range Monaspace Krypton NF → GRUB PF2 build/validation, Homebrew desktop/icon validation, an empty-home KDE/Breeze and DMS environment-boundary assertion, then Aurora's package-free Plasma launcher-default assertion and compiled applet validation plus the Breeze launcher-alias rewrite | after `dnf`, `files`, and its own module 4d font install; the final three validations depend on installed/overlaid files — DD-026, DD-033…DD-046, DD-057, DD-062, DD-063, DD-066…DD-069 |
