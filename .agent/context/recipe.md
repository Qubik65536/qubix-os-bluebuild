...
- Fcitx is **packaged, not compiled** (DD-050). `fcitx5-chinese-addons` supplies Pinyin;
  ...
  non-repeating/non-inhibitable `Ctrl+space` binding with its deterministic engine-switch
  helper, KWin selection, and Wayland GTK environment correction live in the overlay. The
  last unsets Fedora's broad `GTK_IM_MODULE` on Wayland and retains GTK 3/4 X11 fallback
