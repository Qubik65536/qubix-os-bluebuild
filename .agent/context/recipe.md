...
- **Five modules, executed in file order:**
  ...
  |---|---|---|---|
  | 1 | `files` | Copies `files/system/*` → `/` | none (kept first by convention) |
  | 2 | `dnf` | COPR `atim/starship`; install `micro`, `starship`; remove `firefox`, `firefox-langpacks` | before `default-flatpaks` |
