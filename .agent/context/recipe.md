...
- **Composition:** `from-file: common-base.yml` → `from-file: common-identity.yml` →
  ...
  | 4 | `containerfile` | `common-base.yml` | Ten snippets: the existing nine terminal/setup assertions, then GRUB theme font conversion, source/marker/timeout/reverse-canvas-order checks, and manifest generation | after `dnf` and `files`; GRUB generation needs `grub2-mkfont`, IBM Plex Mono, and the overlaid source — DD-026, DD-033…DD-046, DD-057 |
