...
- **Composition:** `from-file: common-base.yml` → `from-file: common-identity.yml` →
  ...
  | 4 | `containerfile` | `common-base.yml` | Ten snippets: the existing nine terminal/setup assertions, then bounded-range Monaspace Krypton NF → GRUB PF2 conversion, source/marker/numeric-timeout/no-progress-widget/reverse-canvas-order checks, and manifest generation | after `dnf`, `files`, and its own module 4d font install; GRUB generation needs `grub2-mkfont`, Krypton NF Regular/Bold, and the overlaid source — DD-026, DD-033…DD-046, DD-057 |
