...
| `design-decisions.md` | `DD-001`…`DD-064` — every "why" in the project | Before questioning any convention |
...
| `desktops.md` | The two sessions (Plasma, Niri), switching, the complete Breeze Dark fallback for fresh accounts and KDE Qt integration in both sessions (DD-063), Homebrew application discovery, stable cask icons, and automatic launcher refresh (DD-062), Niri config and keybinds, default terminal/browser, portals, Simplified Chinese input, the Niri/DMS theme, display scale, and troubleshooting | Touching either desktop session |
...
| `branding.md` | Asset → image path → consumer map, including the clean visual `Qubix OS` versus technical BlueBuild identity split (DD-064); GRUB delivery and layout; Plymouth/initramfs boundary; Plasma splash; logo-change procedure | Touching branding or product identity |
| `build-and-release.md` | Weekly OCI and automatic/manual ISO workflows, digest pinning, Lorax build-stamp visual branding (DD-064), the offline Flatpak repository and pinned `umoci` shim (DD-061), permissions, signing, OneDrive retention, release indexing, capacity, and failure triage | Changing CI or debugging a build |
