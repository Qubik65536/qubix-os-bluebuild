...
| `design-decisions.md` | `DD-001`…`DD-073` — every "why" in the project, including source-stamped system identity and timestamp-prefixed ISO release tags | Before questioning any convention |
...
| `build-and-release.md` | Weekly OCI and automatic/manual ISO workflows, source-revision stamping (DD-073), digest pinning, Anaconda pre-start product branding (DD-065), the offline Flatpak repository and pinned `umoci` shim (DD-061), permissions, signing, OneDrive retention, timestamp-ordered release indexing, capacity, and failure triage | Changing CI or debugging a build |
