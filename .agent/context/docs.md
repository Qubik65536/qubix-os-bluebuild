...
| `design-decisions.md` | `DD-001`…`DD-029` — every "why" in the project | Before questioning any convention |
...
| `shell.md` | The terminal environment: what is installed, how it is delivered as system files with nothing written to `$HOME` (DD-026, DD-030), why zsh is wired from `/etc/zshenv` and what that costs, per-tool notes for starship/atuin/plugins/bat/yazi/lazygit/zellij/fastfetch, the one `chsh` an existing account needs, the `git clone` that installs LazyVim and the two ways to update it, and how to override any of it | Touching anything a shell reads |
