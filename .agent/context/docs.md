...
| `design-decisions.md` | `DD-001`…`DD-021` — every "why" in the project | Before questioning any convention |
...
| `desktops.md` | The two sessions (Plasma, Niri), switching, Niri config and keybinds, default terminal, and the "Niri session shows nothing" procedure, which leads with the Xwayland Video Bridge and then splits a stale panel from a dead compositor from a dead shell (IMG-012) | Touching either desktop session |
