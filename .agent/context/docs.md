...
| `design-decisions.md` | `DD-001`…`DD-070` — every "why" in the project | Before questioning any convention |
...
| `desktops.md` | The two sessions (Plasma, Niri), switching, complete Breeze Dark and DMS-safe KDE Qt integration (DD-063, DD-066), desktop-aware Fcitx environment split (DD-067), Aurora's package-free Plasma launcher defaults and Qubix panel launcher (DD-068, DD-069), Quickshell's targeted Qt runtime synchronization and loader troubleshooting (DD-070), Homebrew application discovery/refresh (DD-062), Niri config/keybinds, default terminal/browser, portals, Simplified Chinese input, theme, scale, and troubleshooting | Touching either desktop session |
