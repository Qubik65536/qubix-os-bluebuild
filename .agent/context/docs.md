...
| `design-decisions.md` | `DD-001`…`DD-057` — every "why" in the project | Before questioning any convention |
...
| `branding.md` | Asset → image path → consumer map, grouped by source artwork; GRUB's `/usr`→`/boot` delivery and opt-out; Plymouth/initramfs boundary; Qubix-native Plasma splash plus Aurora compatibility paths; per-user splash recovery; logo-change procedure; logo green vs. the Niri accent | Touching anything under `files/system/` |
| `build-and-release.md` | Weekly OCI and automatic/manual ISO workflows, trigger chaining, permissions, signing, artifacts, tags, failure triage | Changing CI or debugging a build |
| `usage.md` | Install, update, rollback, verify, automatically or manually build/download/check an ISO artifact, uninstall | Answering a user-facing question |
