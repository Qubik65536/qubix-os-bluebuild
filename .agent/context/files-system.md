...
| `etc/niri/config.kdl` | niri | System-default session config; DMS keybinds (`Mod+Space` launcher) plus a non-repeating, non-inhibitable `Ctrl+Space` invoking `/usr/bin/fcitx5-remote -t`; explicitly unsets `GTK_IM_MODULE` for compositor-launched clients; `eDP-1` pinned to `scale 1`; window rule hiding the Xwayland Video Bridge; `include`s the theme (DD-014, DD-015, DD-019, DD-024, DD-050) |
