...
| `etc/profile.d/zz-qubix-fcitx-wayland.sh` | graphical sessions and interactive shells | Sorts after Fedora's `fcitx5.sh`; on Wayland it always unsets GTK, then uses SDDM's desktop identity to unset Qt/SDL only for Plasma. Niri retains Qt and both retain XIM. Module 4l separately patches Plasma's parent session command because its child-shell environment loader cannot propagate an `unset` (DD-050, DD-067) |
...
| `etc/fastfetch/config.jsonc` | fastfetch, when a user runs it | The system-wide default box. **The only system-wide path fastfetch reads** — its search path has no `/usr` entry. Its pinned Fedora logo remains deterministic across the clean visual `NAME` / technical `ID` split (DD-031, DD-041, DD-065) |
