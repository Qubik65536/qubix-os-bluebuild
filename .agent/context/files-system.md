...
| `etc/profile.d/zz-qubix-fcitx-wayland.sh` | graphical sessions and interactive shells | Sorts after Fedora's `fcitx5.sh`; on Wayland it always unsets GTK, then uses the display manager's desktop identity to unset Qt/SDL only for Plasma. Niri retains Qt and both retain XIM. Module 4m separately patches Plasma's parent session command because its child-shell environment loader cannot propagate an `unset` (DD-050, DD-067) |
...
| `usr/lib/environment.d/50-qubix-terminal.conf` | systemd user manager | `TERMINAL=wezterm`, `/etc/xdg` in `XDG_CONFIG_DIRS`, and Homebrew's shared prefix in `XDG_DATA_DIRS`. It deliberately does **not** export KDE's Qt platform theme: doing so reaches and breaks the independent DMS service (DD-066). Search paths preserve inherited values; `etc/profile.d/qubix-shell-env.sh` carries the shell half (DD-038, DD-062) |
...
| `etc/niri/config.kdl` | niri | System-default session config; DMS keybinds consume `Mod+Space` for the launcher but deliberately leave `Ctrl+Space` to Fcitx; unsets `GTK_IM_MODULE` and gives compositor-launched KDE apps `QT_QPA_PLATFORMTHEME=kde`; `eDP-1` pinned to `scale 1`; window rule hiding the Xwayland Video Bridge; `include`s the theme (DD-014, DD-015, DD-019, DD-024, DD-050, DD-066) |
...
| `usr/lib/systemd/user/dms.service.d/50-qubix-environment.conf` | systemd user manager | Strips `QT_QPA_PLATFORMTHEME` from DMS itself so the Qt/Quickshell shell cannot inherit Plasma's application integration, then uses DMS's supported default launch prefix to restore it only for applications the shell opens. A personal DMS `launchPrefix` wins; this cannot live on `niri.service` because the wanted service is a sibling unit (DD-066) |
...
| Path | Consumer | Effect |
|---|---|---|
| `usr/share/qubix-os/grub-theme/` | `qubix-grub-theme`, then GRUB `gfxmenu` from `/boot` | Immutable Qubix Boot Console source: approved 1920×1080 wireframe-cube background, percentage layout, solid UI primitives and protocol `VERSION` (now 2 for Krypton asset names). GRUB paints components in reverse source order, so the opaque panel is deliberately the final component in `theme.txt`; moving it first covers the entire live menu. Timeout is one live numeric label—GRUB's progress widget is deliberately absent because its 200×28 minimum breaks the compact footer. Module 4j asserts both choices and adds three bounded-range Monaspace Krypton NF PF2 fonts plus `MANIFEST.sha256` (DD-057) |
| `usr/bin/qubix-grub-theme` | systemd or root by hand | Validates the source, atomically stages it under content-addressed `/boot/grub2/themes/qubix-v2-*`, and atomically replaces only its delimited block in `custom.cfg`. Sets an eight-second visible menu; never runs `grub2-mkconfig` or emits entries. `--check` is read-only; `--remove` leaves inert assets (DD-057). Mode `100755` |
...
| `etc/fastfetch/config.jsonc` | fastfetch, when a user runs it | The system-wide default box. **The only system-wide path fastfetch reads** — its search path has no `/usr` entry. Its pinned Fedora logo remains deterministic across the clean visual `NAME` / technical `ID` split, and its Linux display-manager field is provider-agnostic (DD-031, DD-041, DD-065) |
