...
| `usr/share/qubix-os/distrobox/qubix-distrobox.zsh` | zsh inside the guest, at login | The guest's interactive zsh setup: reads plugins, starship, atuin, and helpers **from the host** via `/run/host/usr/share/qubix-os/…`; it uses `compinit -C` because `compaudit` cannot verify that bind-mount path (DD-044). A host rebase updates it with no action in the guest |
